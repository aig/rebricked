...
# The Rebricked mark - the "RE" chip, the same brand device used in the sidebar (index.html).
...
FAVICON = (
    ...
    "<rect x='3' y='7' width='26' height='18' rx='4' fill='%23FF3621'/>"
    "<text x='16' y='22' text-anchor='middle' fill='%23fff' "
    "font-family='Segoe UI,system-ui,sans-serif' font-size='14' font-weight='800'>RE</text>"
    "</svg>"
)
...
def badge_emblem(tier):
    ...
    return (
        ...
        '<rect x="74" y="34" width="52" height="40" rx="9" fill="#fff"/>'
        '<text x="100" y="65" text-anchor="middle" fill="#0E1116" '
        'font-family="Segoe UI, system-ui, sans-serif" font-size="26" font-weight="800" letter-spacing="-1">RE</text>'
...
(function () {
  ...
  var surface = document.querySelector('.post-doc') ? 'guide'
    : document.querySelector('.learn-doc') ? 'learn-index'
    : document.querySelector('.entry-doc') ? 'entry'
    : document.querySelector('.hub-doc') ? 'hub'
  ...
  if (s) s.addEventListener('keydown', function (e) {
    if (e.key === 'Enter') {
      var v = s.value.trim();
      if (v) track('search', { term: v, surface: surface });
      location.href = '../../' + (v ? ('?q=' + encodeURIComponent(v)) : '');
    }
  });
  // One delegated listener for every link that says something about intent. Cheaper than
  // per-element handlers and it survives template changes, since it matches on the classes
  // the generators already emit.
  document.addEventListener('click', function (e) {
    var a = e.target.closest && e.target.closest('a');
    if (!a) return;
    var href = a.getAttribute('href') || '';
    if (a.closest('.post-toc')) return track('guide-toc', { slug: slug, to: href });
    if (a.closest('.entry-sources')) return track('source-click', { surface: surface, slug: slug, url: href });
    if (a.closest('.entry-related')) return track('related-click', { surface: surface, slug: slug, to: href });
    if (a.closest('.post-prevnext')) return track('guide-prevnext', { slug: slug, to: href });
    if (a.closest('.learn-row')) return track('guide-open', { to: href, source: 'learn-index' });
    if (a.classList.contains('hub-link')) return track('hub-entry-open', { to: href });
    if (a.classList.contains('entry-cta')) return track('cta-click', { surface: surface, slug: slug, to: href });
    if (a.classList.contains('quiz-cta')) return track('quiz-open', { source: surface });
    if (a.classList.contains('badge-intro-cta')) return track('quiz-open', { source: 'badge-page' });
    if (a.id === 'gh-top') return track('github-click', { surface: surface });
    if (a.id === 'rss-top') return track('rss-click', { surface: surface });
    // The inline 🔗 citation chips on an entry card - the entry-page equivalent of a
    // guide's prose link, and the signal for which sourced claim readers go verify.
    if (a.classList.contains('date-src')) return track('source-click', { surface: surface, slug: slug, url: href });
    // Anything else leaving for an official doc: the citations readers actually follow.
    if (a.closest('.post-body') && /^https?:/i.test(href)) {
      return track('guide-outbound', { slug: slug, url: href });
    }
  });
  // Read depth, guides only - the one metric that tells a blog post from a bounce. Fires each
  // quarter once, on a passive listener so it cannot cost scroll performance.
  if (surface === 'guide') {
    var seen = {};
    window.addEventListener('scroll', function () {
      ...
      [25, 50, 75, 100].forEach(function (m) {
        if (pct >= m && !seen[m]) { seen[m] = 1; track('guide-read-depth', { slug: slug, percent: m }); }
      });
    }, { passive: true });
  }
  ...
  // Current links carry a single ?name; older shared links used ?first/?last.
  var name = (p.get('name') || '').trim(), first = (p.get('first') || '').trim(), last = (p.get('last') || '').trim();
  var recipient = (name || [first, last].filter(Boolean).join(' ')).replace(/\\s+/g, ' ').slice(0, 101);
  ...
  if (share) share.addEventListener('click', function () {
    ...
    track('badge-share', { badge: slug, named: !!recipient });
    window.open('https://www.linkedin.com/sharing/share-offsite/?url=' + encodeURIComponent(url), '_blank', 'noopener,noreferrer');
  });
  var pdf = document.getElementById('badge-export-pdf');
  if (pdf) pdf.addEventListener('click', function () { track('badge-export', { badge: slug, format: 'pdf' }); window.print(); });
  ...
  if (jpg) jpg.addEventListener('click', async function () {
    ...
    track('badge-export', { badge: slug, format: 'jpg' });
