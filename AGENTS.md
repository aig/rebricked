...
| `www/databricks.features.json` | Generated by `build_features.py` from `kb/databricks/` (gitignored). The array the app fetches at runtime; entries sorted by `id`, since `app.js` sorts client-side anyway. Don't hand-edit - your change would be overwritten on the next build and never reach the site. |
...
1. Build the data from `kb/`, then run the schema gate - both must pass (CI runs the same two).
   The validator reads the *built JSON*, so skipping the build validates a stale file:
   ```
...
3. Commit the `kb/` YAML, **not** the built JSON - `www/databricks.features.json`,
   `www/databricks/`, `sitemap.xml`, and `feed.xml` are all gitignored build output. A diff
   should show only the entry files you actually touched.
