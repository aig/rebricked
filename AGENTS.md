...
| [`agents/`](agents/) | Task-scoped skills. [`add-databricks-entry.md`](agents/add-databricks-entry.md): the skill to follow when adding **or editing** any entry - investigate a Databricks thing's history + status, then add/correct the right-kind, sourced entry and validate. Tool-agnostic; sibling of this file. |
