...
publishes (the CI uploads `www`, nothing else). Repo docs, `scripts/`, `agents/`, and
`reference/` stay outside it and are never deployed.

| File | What it is |
|------|------------|
| [`www/databricks.features.json`](www/databricks.features.json) | **The data. Source of truth.** An array of rename, deprecation, and feature objects. Lives in `www/` because the app fetches it at runtime. |
...
| `www/databricks/` | Generated by `build_entries.py` (gitignored). The vendor hub + one crawlable page per entry. Don't hand-edit; run the generator (CI also regenerates on deploy). |
| `www/sitemap.xml` / `www/feed.xml` | Generated by `build_entries.py` (gitignored). The crawlable sitemap and an RSS 2.0 feed of every entry. Don't hand-edit. |
...
## Data shape (`databricks.features.json`)
...
id, never re-slugged. Date *tokens* are `YYYY` or `YYYY-MM` (`verified` is `YYYY-MM-DD`), but
...
3. If you changed the sidebar, keep [`app.js`](www/app.js)'s `NAV` config in sync: each rail
   item maps to the entries it covers via an `ids` array (entry `id`s from `databricks.features.json`,
