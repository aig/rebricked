...
### Added
- **A badge page per quiz result** — `badges/<n>-of-5/` for every score 0–5, each a
  standalone page with a funny "achievement" (e.g. 5/5 = "Keeper of the Renames",
  0/5 = "Rip Van Rebrand"), a score meter, and Open Graph tags so the badge shows in a
  link preview. Generated by [`scripts/build_badges.py`](scripts/build_badges.py) (rerun
  after editing the copy); the folders are committed so there's still no deploy-time build.
